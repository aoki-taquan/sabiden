...
|---|---|---|
| PWA → PWA | ✅ full multi-leg orchestration | **integration**: `tests/intercom_integration.rs::rfc5853_pwa_to_pwa_dispatcher_e2e_no_ngn_traffic_and_bridge_forwards_media` (`handle_pwa_outbound_offer` 経由で実 dispatcher → 実 WS Offer push → 実 bridge attach → caller→callee MediaFrame 配送、 NGN socket 到達 0 件を確認) |
