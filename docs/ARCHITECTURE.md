...
│   ├── intercom.rs   # 内線間 direct dial (Issue #313): classify_dial_target / InternalCallRegistry / IntercomService / WebRtcRelay bridge helper + ExtensionAliases (Issue #315 [dial_aliases] numeric→alphabetic AOR alias map)
...
- `PushSubscription { endpoint, p256dh, auth }` ── 1 device の購読単位。
  `validate()` で HTTPS scheme / base64url (**RFC 7515 §2 / RFC 8291 §3.2、
  no-padding 必須**) / 空鍵を検査。 padding (`=`) 入り base64url は
  Issue #307 で reject 強制 (PWA frontend が `=` を strip するので、 server
  が `=` を許容すると同 device で 2 重 entry が登録され fan-out が 2 重化する)。
...
- `PushNotifier` trait + `WebPushNotifier` 実装 ── 本番は `IsahcWebPushClient`
  (HTTP/2)。 起動時に `PartialVapidSignatureBuilder` を **1 度だけ** PEM から
  パースして保持し、 通知ごとに `.clone()` + `add_sub_info(&info)` で
  per-subscription JWT を生成する (Issue #308: fan-out 数が多い時の ECDSA
  decode コスト削減、 web-push crate `vapid/builder.rs` §270 `#[derive(Clone)]`)。
  test は `MockPushNotifier` で fan-out / Gone 経路を検証。
- `PushSendError` enum (`Gone` / `Rejected { code, message }` / `Transient(String)`)
  ── RFC 8030 §5 status code を 3 区分で表現 (Issue #309):
  - **Gone** (404 / 410): subscription が永続的に無効。 store から即削除。
  - **Rejected { code, message }** (400 BadRequest / 413 PayloadTooLarge /
    InvalidCryptoKeys 等、 request 内容が恒久的に不正): store は保持
    (config 修正 + 再起動で復活可能性あり)。 `code` は HTTP status 由来の
    数値 (crate 内 validation error は `code = 0`)、 metric / log 区別用。
  - **Transient(String)** (401 Unauthorized / 5xx ServerError / IO):
    一時障害、 retry 視野で store 保持。 Unauthorized (= VAPID JWT exp 切れ /
    署名鍵 rotation) は Issue #309 で意図的に Transient に分類 (再生成で復活)。
...
5. `Gone` (404/410) は store から自動削除 (RFC 8030 §5)。
   `Rejected { code }` と `Transient(_)` は subscription 保持 (前者は config
   修正待ち、 後者は次回送信での復活期待、 Issue #309)。
...
> **scope (Issue #315 で alphabetic AOR 制約を解除済)**: PWA から内線 dial する
> 経路は WS 入口の [`is_valid_dial_target`] (charset `[0-9*#+]{1,32}`、 RFC 3261
> §25.1 user 文法のサブセット、 CRLF injection 防御) を通過する必要がある。
> alphabetic AOR (`"alice"` / `"iphone"` 等) はそのままでは WS validator で
> `invalid_target` として弾かれるため、 **Issue #315 で
> [`[dial_aliases]`](#alphabetic-aor-対応-dial_aliases-alias-map-issue-315)
> alias map (= numeric → alphabetic AOR の対応表) を導入**し、 PWA は
> numeric (`"101"`) で dial → sabiden 内部で alphabetic AOR (`"alice"`) に解決
> する経路に揃えた。 WS validator は引き続き digit-only で運用 (CRLF injection
> / SIP smuggling 防御は破らない)。
>
> dispatcher 単体 (= classify_dial_target / bridge attach) は AOR 文字種に
> 制約を持たないため、 lib unit test / trait-API direct integration test では
> `"alice"` 等の alphabetic AOR を直接渡して挙動を検証している。 production
> 到達経路 (WS 入口経由) は alias map を介すか numeric AOR 直登録で対応する。
>
> **SIP UA caller 経路 (Issue #317)** は WS validator を通らず、 sabiden
> ExtensionUas が Request-URI の user 部をそのまま `classify_dial_target` に
> 流す。 SIP UA は REGISTER 時に好きな AOR (= numeric / alphabetic) を取得
> できるため、 charset 制約は無い (RFC 3261 §25.1 SIP URI user 文法のみ)。

#### dispatcher (`classify_dial_target`)

```text
caller (PWA / SIP UA)
       │ INVITE target = "101" (numeric AOR / alias) or "0312345678" (NGN 番号)
       ▼
...
sabiden orchestrator
       │
       ▼  classify_dial_target(target, ExtensionRegistrar, ExtensionAliases)
       │
       │  Step 1: alias resolve (Issue #315)
       │   aliases.resolve(target) → Some("alice") なら AOR = "alice"
       │                            → None なら AOR = target (旧挙動)
       │  Step 2: registrar.lookup(AOR)
       │
       ├── Internal { binding, aor }  ──► 内線間 dial へ分岐 (本セクション)
       └── Ngn { target }             ──► 既存 NGN プロキシ経路 (`docs/asterisk-real-invite.md` §5)
                                          (NGN へは 元の target 文字列 を渡す、 alphabetic は NGN に流さない)
```

PWA 経路では WS 入口 [`is_valid_dial_target`] が先に立つので、 dial 文字列は
digit-only (`[0-9*#+]{1,32}`)。 alphabetic AOR (`"alice"` 等) には
**[`[dial_aliases]`](#alphabetic-aor-対応-dial_aliases-alias-map-issue-315)**
の alias 解決を経由して到達する (Issue #315、 例: `"101"` → `"alice"`)。 SIP UA
経路 (`handle_invite`) には WS validator はない (= SIP message 自体の
Request-URI parse でホスト/パラメータが分離されるため、 任意 user 部が直接
dispatcher に来る)。 SIP UA → 内線 AOR は Issue #317 で full multi-leg
orchestration が実装され、 `enabled = true` 構成では
`dispatch_sip_ua_internal_invite` ヘルパが callee binding の `transport` で
case A (SIP UA → SIP UA、 `ext_inviter.invite_intercom` + `RtpBridge` で PCMU
リレー) と case B (SIP UA → PWA、 `peer.create_offer` → callee WS push →
answer 受信 → `WebRtcAudioBridge` direct_pcmu_passthrough) を分岐する。
`enabled = false` (= キルスイッチ) のときは 480 Temporarily Unavailable で
fail-fast し、 NGN への内線 AOR 漏洩を防ぐ。

判定ルール (Issue #315 で 2 step に拡張):

| alias hit | registrar hit | 結果 |
|---|---|---|
| Yes (`101 → alice`) | Yes (`alice` 登録済) | `Internal { aor: "alice", binding }` |
| Yes (`101 → alice`) | No (`alice` 未登録 / 期限切れ) | `Ngn { target: "101" }` (旧挙動、 元の dial 文字列で NGN へ) |
| No | Yes (`alice` をそのまま AOR、 登録済) | `Internal { aor: "alice", binding }` (SIP UA caller) |
| No | No | `Ngn { target }` (旧挙動) |

AOR が「存在しない内線番号」 のケース (例 `"99999"` だが registrar 未登録) は
NGN へ流れて 404 で帰る。 これは sabiden 側が「内線番号文法」 を解釈する
band-aid を避ける設計選択 (CLAUDE.md §6.1)。 alias hit + registrar miss でも
NGN へは **元の target 文字列** (`"101"`) を載せて流し、 alphabetic AOR
(`"alice"`) を NGN Request-URI に出さない (NGN は alphabetic を 404 で弾く)。
...
#### alphabetic AOR 対応 (`[dial_aliases]` alias map、 Issue #315)

PWA WS validator [`is_valid_dial_target`] は CRLF injection / SIP smuggling
防御 (PR #146 review #1 🔴#1) のため digit-only (`[0-9*#+]{1,32}`、 RFC 3261
§25.1 user 文法サブセット) を強制する。 このため alphabetic AOR (`"alice"`
等) を PWA から直接 dial できない制約があった (PR #314 までの scope)。

Issue #315 で **`[dial_aliases]` alias map** を導入し、 PWA は引き続き
numeric (`"101"`) で dial、 sabiden 内部で alphabetic AOR (`"alice"`) に
解決する経路に揃えた。 WS validator は **据え置き** (= digit-only のまま、
...
| alias 設定 | dial | 解決 AOR | registrar.lookup |
|---|---|---|---|
| `"101" = "alice"` | `"101"` (PWA) | `"alice"` | `alice` 登録済なら Internal |
| `"101" = "alice"` | `"101"` (PWA) | `"alice"` | `alice` 未登録なら Ngn (元 `"101"` で NGN へ) |
| 未設定 (= 空 map) | `"101"` (PWA) | `"101"` (旧挙動) | `101` 登録済なら Internal、 そうでなければ NGN |
| 未設定 | `"alice"` (SIP UA caller) | `"alice"` (旧挙動) | SIP UA caller 経路は WS validator 通らないため alphabetic 直渡し可能 |

設計選択 (Issue #313 本文 §5、 #315 で採用):

1. **option A (WS validator 拡張)** — `is_valid_dial_target` charset に
   `[a-zA-Z]` を追加する案。 CRLF injection 防御の defense-in-depth を
   弱め (`@` / `;` 等の追加 escape 検討が必要)、 SOHO PBX としては
   「内線番号 = 数字」 UX が直感的でない、 という理由で**不採用**。
2. **option B (alias map)** — 本 PR で採用。 WS validator は据え置きで
   numeric 制約を維持し、 alias で「numeric ↔ alphabetic」 を bridge する。
   `[dial_aliases]` セクション未設定時は完全な旧挙動 (= target をそのまま
   AOR として lookup) を維持するため、 後方互換性も保たれる。

注入箇所 (`main.rs`):

```rust
let aliases = Arc::new(full_config.dial_aliases.clone());
uas_handler.set_dial_aliases(aliases).await;
```

`Mutex<Arc<ExtensionAliases>>` で内側差し替え式 (`set_intercom_service`
パターンと一致)。 `[dial_aliases]` セクション省略時は
`ExtensionAliases::empty()` (= 空 HashMap) を注入する。
...
5. **SIP UA → 内線 AOR の dispatcher gate**: `handle_invite` 冒頭で
   classify を呼び、 Internal hit なら `dispatch_sip_ua_internal_invite` ヘルパ
   に振り分ける (Issue #317)。 `enabled = false` (= キルスイッチ) のときのみ
   **480 Temporarily Unavailable** (RFC 3261 §21.4.18) で fail-fast し、 NGN に
   内線 AOR が漏れる band-aid を遮断する (CLAUDE.md §6.1)。
6. **PWA → SIP UA full multi-leg orchestration** (Issue #316、 PR #320 で landing):
   ...
   送出する。
7. **SIP UA → 内線 (SIP UA / PWA) full multi-leg orchestration** (Issue #317、
   PR で landing): `handle_invite` の dispatcher で classify が Internal hit
   と判定したとき、 `dispatch_sip_ua_internal_invite` ヘルパが callee binding の
   `transport` で 2 case を分岐する:
   - **case A (SIP UA → SIP UA)**:
     (a) caller の PCMU AVP SDP offer (RFC 3551 PT 0) から RTP endpoint 抽出、
     (b) sabiden 内線 NIC に caller-leg / callee-leg 2 つの RTP socket を bind、
     (c) caller SDP を `Negotiator::for_ngn` で PCMU only に正規化し、 c=/m= を
     sabiden callee-leg socket addr に書換え (RFC 4566)、
     (d) `LegInviter::invite_intercom` で callee SIP UA に INVITE 送出 (RFC 5626、
     destination = `binding.remote`)、 (e) 200 OK SDP から callee RTP endpoint
     を抽出、 (f) callee answer に同じ正規化を掛け c=/m= を sabiden caller-leg
     socket addr に書換えた SDP を caller に **200 OK** で返却 (B2BUA SDP
     anchoring、 RFC 3261 §13.2)、 (g) `MediaBridge::Relay` (`RtpBridge`、
     既存 PCMU↔PCMU リレー) を caller-leg / callee-leg socket 間で起動、
     (h) `IntercomService::register_call` で `InternalCallRegistry` 登録。
   - **case B (SIP UA → PWA)**:
     (a) caller SDP から RTP endpoint 抽出、 sabiden 内線 NIC に caller-leg
     RTP socket を bind、 (b) callee PWA `peer.create_offer` で SAVPF offer
     生成 (RFC 8829 §5.2)、 (c) callee `ws` に `ServerMessage::Offer { call_id, sdp }`
     を push し `PendingAnswers` で answer 待ち (30 秒 timeout、 RFC 3261 §13.2)、
     (d) `peer.accept_answer` で DTLS-SRTP 確立 (RFC 8829 §5.3)、 (e) callee
     `peer.take_media_rx` で MediaFrame source 取得、 (f) callee SAVPF answer を
     AVP→PCMU only に正規化し c=/m= を sabiden caller-leg socket addr に書換え
     て caller に **200 OK** 返却、 (g) `WebRtcAudioBridge`
     (`direct_pcmu_passthrough = true`) を caller-leg UDP socket ⇄ callee PWA
     peer 間で起動、 (h) `IntercomService::register_call` で登録。
   失敗時の caller への応答コード: admit 拒否 → 486、 socket bind / SDP rewrite
   失敗 → 500、 callee 3xx-6xx → same-status echo (RFC 3261 §13.2.2.3)、
   transport error / answer timeout → 408、 200 OK 後 bridge attach 失敗 →
   case A は callee dialog に BYE 送出 (RFC 3261 §15.1.1) / case B は
   `callee_peer.close()` (RFC 8829 §5.1)。

本 PR review follow-up Issue で残務 (= まだ実装されていない経路):

8. **PWA→SIP UA / SIP UA→ 内線 経路の BYE 連動**: 現状は callee SIP UA 側から
...
9. ~~**alphabetic AOR 対応 (PWA 側)**~~ — **Issue #315 で解決済**。 PWA は
   numeric (`"101"`) で dial、 sabiden 内部で
   [`[dial_aliases]`](#alphabetic-aor-対応-dial_aliases-alias-map-issue-315)
   alias map により alphabetic AOR (`"alice"` 等) に解決する経路を実装した
   (option B)。 WS validator は digit-only 据え置きで CRLF injection / SIP
   smuggling 防御 (PR #146 review #1 🔴#1) は破らない。

#### Self-call gate (Issue #324、 RFC 3261 §16 / §21.4.18)

SIP UA → 内線 AOR の dispatcher gate (`handle_invite` 冒頭) は、
**`target_user == from_aor`** (= 自分自身が自分の AOR に INVITE) **または**
**`resolved_target == from_aor`** (= alias 解決後 AOR が自分自身、 Issue #315
統合) を `classify_dial_target` を通す手前で **480 Temporarily Unavailable**
で fail-fast する。 旧コードでは self-call を Internal 経路の skip 条件として
...
INVITE (target_user)
       │
       ▼
resolved_target = dial_aliases.resolve(target_user).unwrap_or(target_user)
       │
       ▼
handle_invite intercom gate
       │
       ├── resolved_target == from_aor OR target_user == from_aor ──► 480 Temporarily Unavailable
       │   (RFC 3261 §21.4.18 / §16 loop detection、 NGN 漏洩防止、 Issue #324 / #315)
       │
       └── それ以外 ─────────────────────────────────────────────►  classify_dial_target → 通常経路
```
...
NGN 漏洩しない最低限の保証)。 Issue #315 alias map (例: `[dial_aliases]
"101" = "alice"`) を入れた構成では、 `from_aor = "alice"` で numeric dial
`"101"` を打った場合も alias 解決後 AOR (`"alice"`) で self-call と判定される。
...
#### SIP UA → SIP UA full multi-leg シーケンス (Issue #317 case A)

```text
SIP UA (caller, alice)        sabiden                              SIP UA (callee, bob via REGISTER)
  │                              │                                          │
  │ INVITE sip:bob@sabiden       │                                          │
  │   c=caller IP / m=caller RTP port (PCMU AVP)                            │
  ├─────────────────────────────►│                                          │
  │                              │ ExtensionUas → UasEvent::Invite          │
  │                              │ extract_user_from_sip_uri → "bob"        │
  │                              │ classify_dial_target → Internal{Sip,..}  │
  │                              │ try_admit (capacity OK)                  │
  │                              │ extract_rtp_endpoint(caller SDP)         │
  │ 100 Trying                   │                                          │
  │◄─────────────────────────────┤                                          │
  │                              │ UdpSocket::bind × 2 (caller-leg, callee-leg)│
  │                              │ Negotiator::for_ngn(rewrite caller SDP)  │
  │                              │ rewrite_rtp_endpoint(callee-leg addr)    │
  │                              │                                          │
  │                              │ Uac::invite_to(dest=binding.remote)      │
  │                              │   plan = sip:bob@<addr> (PCMU SDP offer) │
  │                              ├─────────────────────────────────────────►│
  │                              │                                          │ INVITE 受信 / 200 OK 組立
  │                              │◄─────────────────────────────────────────┤ 200 OK + PCMU SDP
  │                              │ ACK 自動送出 (RFC 3261 §13.2.2.4)         │
  │                              ├─────────────────────────────────────────►│
  │                              │ extract_rtp_endpoint(callee SDP)         │
  │                              │ Negotiator::for_ngn(rewrite callee answer)│
  │                              │ rewrite_rtp_endpoint(caller-leg addr)    │
  │ 200 OK + PCMU SDP            │                                          │
  │   c=caller-leg IP/port (B2BUA anchoring)                                │
  │◄─────────────────────────────┤                                          │
  │ ACK                          │                                          │
  ├─────────────────────────────►│                                          │
  │                              │ RtpBridge::start (MediaBridge::Relay)    │
  │                              │   caller-leg ⇄ callee-leg (PCMU UDP)     │
  │                              │ IntercomService::register_call           │
  │                              │                                          │
  │ (RTP PCMU PT 0)              │ pure UDP relay (transcode 不要)          │ (RTP PCMU PT 0)
  ├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─►│ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─►│
  │◄─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ │◄ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┤
```

#### SIP UA → PWA full multi-leg シーケンス (Issue #317 case B)

```text
SIP UA (caller, alice)        sabiden                              PWA (callee, carol via WS REGISTER)
  │                              │                                          │
  │ INVITE sip:carol@sabiden     │                                          │
  │   c=caller IP / m=caller RTP port (PCMU AVP)                            │
  ├─────────────────────────────►│                                          │
  │                              │ ExtensionUas → UasEvent::Invite          │
  │                              │ classify_dial_target → Internal{WebRtc..}│
  │                              │ try_admit (capacity OK)                  │
  │ 100 Trying                   │                                          │
  │◄─────────────────────────────┤                                          │
  │                              │ extract_rtp_endpoint(caller SDP)         │
  │                              │ UdpSocket::bind (caller-leg)             │
  │                              │ callee_peer.create_offer → SAVPF offer   │
  │                              │ callee_pending.register(call_id)         │
  │                              │ WS ServerMessage::Offer {call_id, sdp}   │
  │                              ├─────────────────────────────────────────►│
  │                              │                                          │ str0m に handle_offer
  │                              │ WS ClientMessage::Answer {call_id, sdp}  │
  │                              │◄─────────────────────────────────────────┤ SAVPF answer
  │                              │ callee_peer.accept_answer (RFC 8829)      │
  │                              │ callee_peer.take_media_rx                │
  │                              │ convert_savpf_to_avp → PCMU only         │
  │                              │ rewrite_rtp_endpoint(caller-leg addr)    │
  │ 200 OK + PCMU SDP            │                                          │
  │   c=caller-leg IP/port (B2BUA anchoring、 AVP for SIP UA caller)        │
  │◄─────────────────────────────┤                                          │
  │ ACK                          │                                          │
  ├─────────────────────────────►│                                          │
  │                              │ WebRtcAudioBridge::start                 │
  │                              │   caller-leg UDP ⇄ sabiden ⇄ callee peer │
  │                              │ direct_pcmu_passthrough = true           │
  │                              │ IntercomService::register_call           │
  │                              │                                          │
  │ (RTP PCMU PT 0)              │ peer.send_media / take_media_rx          │ (str0m PCMU SAVPF)
  ├ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─►│ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─►│
  │◄─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ │◄ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─┤
```
...
| alphabetic AOR (`"alice"` 等) | ✅ SIP UA caller 経路は dispatcher 到達 (WS validator を通らない、 Issue #317 case A/B test で検証済) / ✅ PWA caller 経路は `[dial_aliases]` alias map で numeric→alphabetic 解決 (Issue #315) | integration: `tests/intercom_integration.rs::issue315_ws_entry_numeric_dial_alias_resolves_to_alphabetic_aor_intercom` (WS 入口 `process_client_message` 経由で numeric `"101"` → alias `"alice"` → registrar lookup → callee `"alice"` WS に Offer push、 NGN socket 0 件) + `issue315_alias_hit_but_aor_unregistered_does_not_enter_internal_branch` (alias hit + registrar miss は Internal 分岐に入らず NGN fallback) + lib unit: `call::intercom::tests::issue315_classify_resolves_numeric_to_alphabetic_aor_via_aliases` 等 |
...
また `rewrite_rtp_endpoint` は **RFC 3605 §2 準拠**で `a=rtcp:<port>` 属性を
新 m=audio port + 1 に追従させる (Issue #325)。 `Negotiator::for_ngn(_with_dtmf)`
で先に `a=rtcp:<orig_port+1>` が補完される call sequence (intercom 全経路:
PR #320 / PR #323) では、 続く `rewrite_rtp_endpoint` の port 書換に
合わせて `a=rtcp` も同 port + 1 に再計算しないと、 strict-compliance peer
(PWA / SIP UA) は RTCP を旧 port に送出して sabiden 側 RTCP socket に届かない。
NGN は `a=rtcp:` を honor しない実機検証あり (`docs/asterisk-real-invite.md` §5.2、
2026-05-15 falsification test) のため NGN レッグ自体には影響しないが、 内線
レッグの strict RTCP feedback / RTCP SR (RFC 3550 §6) を正しく届けるため必須。
