...
[`ExtensionRegistrar`](#) に登録済みの AOR (例 `"alice"` / `"iphone"`) に
ヒットする場合、 NGN を介さず内線同士で通話する経路。 NGN ch (= 物理 1 回線)
を消費せず、 NGN 発着信と並列に動く (= multi-line)。

#### dispatcher (`classify_dial_target`)

```text
caller (PWA / SIP UA)
       │ INVITE target = "alice" or "0312345678"
       ▼
...
本 PR (Issue #313) で landing する部分:

1. **Foundation 完成**: `intercom` module (classify_dial_target,
   `InternalCallRegistry`, `IntercomService`, `IntercomConfig`)。
2. **Bridge 完成**: `MediaBridge::WebRtcRelay` variant + `WebRtcRelayBridge` 実装。
3. **Dispatcher 結線**: `UasEventHandler::handle_pwa_outbound_offer` 冒頭で
   intercom dispatcher を起動。 Internal hit 時は NGN 経路を skip し、 PWA に
   pending error を返却 (= "NGN に AOR を投げて 404 band-aid" を断つフェイル
   ファースト措置)。

本 PR 範囲外 (follow-up Issue #313):

4. callee 側 full multi-leg orchestration (PWA→PWA は callee WS Offer push、
   PWA→SIP UA は内線 `ExtInviter` 経路、 SIP UA→PWA は SIP UAS → WS Offer push)。
   foundation は本 PR で完成しており、 wiring のみが follow-up scope。

各経路の双方向 audio / RTP は integration test
(`call::intercom::tests::rfc5853_pwa_to_pwa_intercom_integration_bidirectional`
等) で確認済み。
