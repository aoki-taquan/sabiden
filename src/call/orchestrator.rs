...
impl NgnInboundHandler {
    ...
    async fn handle_invite(&self, request: SipRequest, remote: SocketAddr) -> Result<()> {
        ...
        async move {
            ...
            // 登録済み内線の AOR 一覧を取得し、transport ごとに分岐できる形で渡す
...
/// 登録済み Binding の集合に対し transport ごとの経路で並列に INVITE を撃ち、
/// 最初に 200 OK 相当を返した leg を winner として返す。
///
/// - [`ExtTransport::Sip`] の binding は `inviter` (`LegInviter`) で SIP UAC fork
/// - [`ExtTransport::WebRtc`] の binding は `peer.handle_offer` + WS push 経路
///
/// 同時に複数 leg が走り、`mpsc` で先着順に集める。1 つでも `Established` 相当が
/// 取れたら winner として返し、未確定の他 leg はチャネル drop により暗黙的に
/// abort される (WebRTC leg は CANCEL push を送ってから終了)。
pub async fn fork_to_bindings(
    ...
    // WebRTC leg については Cancel を送るため peer/ws/pending を別保管しておく。
    // (winner 決定後、未終了の leg に向けて Cancel push する)
    ...
    // (browser 側は ServerMessage::Cancel { call_id } を見て呼出 UI を閉じる)。
    if matches!(result, ForkResult::Answered { .. }) {
        let legs = webrtc_legs.lock().await.clone();
        for leg in legs {
            // pending 待機を取り消すと oneshot が drop されて待機側が即終了する。
...
/// 単一の WebRTC leg を実行する: peer に offer を投げ、ブラウザにも push し、
/// browser の Answer を待つ。
...
async fn run_webrtc_leg(
    ...
    // 1) NGN SDP を peer (str0m / stub) に渡して内部状態を進める。
    //    stub は形式チェックのみで answer を返せる。
    ...
    let _peer_answer = match peer.handle_offer(&offer_text).await {
        Ok(a) => Some(a),
        Err(e) => {
            // stub バックエンドは PCMU 0 を含む最小オファに対しても
            // build_minimal_answer が動作する。失敗した場合でも browser 経由で
            // answer を取れるなら継続する設計。
    ...
    // 2) browser からの Answer 受信を予約してから offer を push する
    //    (順序を逆にすると browser が先に answer を送り pending に届かない)。
    ...
    let answer = match tokio::time::timeout(leg_timeout, waiter).await {
        Ok(Ok(sdp)) => sdp,
        Ok(Err(_)) => {
            // sender drop = キャンセル相当 (winner 確定で Cancel 送信されたケース)
    ...
    // 4) browser answer を SipResponse 風に詰めて返す。
...
mod tests {
    ...
    /// バンドエイドだった `webrtc.local` フィルタの代替動作。
    #[tokio::test]
    async fn ngn_invite_to_webrtc_binding_offer_push_and_answer_round_trip() {
        ...
        use crate::webrtc::signaling::{
            ClientMessage, PendingAnswers, ServerMessage, WsSink,
        };
        use std::time::Duration;
        use tokio::sync::mpsc;
        use tokio::time::timeout;
        ...
        let browser_task = tokio::spawn(async move {
            // 1 つ目の ServerMessage が Offer であることを確認、その call_id に対し answer を流す
            ...
            match msg {
                ServerMessage::Offer { call_id, sdp: _ } => {
                    // browser → sabiden の Answer はシグナリング層で deliver される動作を直接エミュレートする
        ...
        assert_eq!(
            answer_body, browser_answer_sdp.as_bytes(),
